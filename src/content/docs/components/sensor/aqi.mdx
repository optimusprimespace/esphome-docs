...
- **calculation_type** (**Required**, enum): The AQI calculation standard to use.
  ...
  - `EAQI`: European Air Quality Index. Returns values from 1 to 6:
    - 1: Good
    - 2: Fair
    - 3: Moderate
    - 4: Poor
...
text_sensor:
  - platform: template
    ...
    id: eaqi_level
    lambda: |-
      if (isnan(id(eaqi).state)) return {};
      switch ((int) id(eaqi).state) {
        case 1: return {"Good"};
        case 2: return {"Fair"};
...
```
> [!NOTE]
> EAQI returns a numeric band from 1 to 6. If you want a human-readable label, add a text_sensor as
> shown above.
